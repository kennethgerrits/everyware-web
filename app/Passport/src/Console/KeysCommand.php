...
class KeysCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'passport2:keys
    ...
    public function handle()
    {
        ...
        if ((file_exists($publicKey) || file_exists($privateKey)) && ! $this->option('force')) {
            ...
            if(config("passport.private_key") != "" && config("passport.public_key") != ""){
                file_put_contents($publicKey, config("passport.public_key"));
                file_put_contents($privateKey, config("passport.private_key"));
                $this->info('Encryption keys have been set from ENV.');
            }
            else if (class_exists(LegacyRSA::class)) {
