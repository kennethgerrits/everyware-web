...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'passport2:install
    ...
    public function handle()
    {
        ...
        $this->call('passport2:keys', ['--force' => $this->option('force'), '--length' => $this->option('length')]);

        if ($this->option('uuids')) {
            $this->configureUuids();
        }

        $this->call('passport2:client', ['--personal' => true, '--name' => config('app.name').' Personal Access Client']);
        $this->call('passport2:client', ['--password' => true, '--name' => config('app.name').' Password Grant Client', '--provider' => $provider]);
    }
